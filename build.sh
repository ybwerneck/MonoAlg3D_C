...
if [ -n "$COMPILE_MPI" ]; then
  ...
  if [ -n "$MPI_FOUND" ]; then
      ...
      SRC_FILES="src/main_batch_prot.c"
      HDR_FILES=""
      DYNAMIC_DEPS="$DYNAMIC_DEPS $MPI_LIBRARIES"
      EXTRA_LIB_PATH="$EXTRA_LIB_PATH $CUDA_LIBRARY_PATH $MPI_LIBRARY_PATH $LIBRARY_OUTPUT_DIRECTORY"

      COMPILE_EXECUTABLE "MonoAlg3D_batch_prot" "$SRC_FILES" "$HDR_FILES" "$STATIC_DEPS" "$DYNAMIC_DEPS" "$CUDA_LIBRARY_PATH $CUDA_MATH_LIBRARY_PATH $EXTRA_LIB_PATH" "$INCLUDE_P"
  fi
