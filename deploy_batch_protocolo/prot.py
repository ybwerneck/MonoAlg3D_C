...
number_volumes = """ + str(number_volumes) + """
...
parser.add_argument('--sigma', action = 'store', dest = 'sigma', default = '0.0001368421', required = False, help = 'Condutividade na malha')
...
parser.add_argument('--mesh_file', action = 'store', dest = 'mesh_file', default = 'meshes/teste_septo.alg', required = False, help = 'Caminho até a malha usada')
...
parser.add_argument('--min_x', action = 'store', dest = 'min_x', default = '72100', required = False, help = 'Coordenada inferior do ponto de aplicação do estímulo no eixo x ')
parser.add_argument('--max_x', action = 'store', dest = 'max_x', default = '74100', required = False, help = 'Coordenada superior do ponto de aplicação do estímulo no eixo x ')
parser.add_argument('--min_y', action = 'store', dest = 'min_y', default = '36700', required = False, help = 'Coordenada inferior do ponto de aplicação do estímulo no eixo y ')
parser.add_argument('--max_y', action = 'store', dest = 'max_y', default = '38700', required = False, help = 'Coordenada superior do ponto de aplicação do estímulo no eixo y ')
...
parser.add_argument('--output_dir_save', action = 'store', dest = 'output_dir_save', default = 'outputs/saveSepto11/', required = False, help = 'Diretório onde os arquivos do save_state vão ser armazenados')
parser.add_argument('--output_dir_restore', action = 'store', dest = 'output_dir_restore', default = 'outputs/restoreSepto11/', required = False, help = 'Diretório onde os arquivos do restore_state vão ser armazenados')
...
parser.add_argument('--nome_arquivo', action = 'store', dest = 'nome_arquivo', default = 'resultadoSepto3.txt', required = False, help = 'Nome do arquivo onde são salvos os resultados do protocolo')
...
parser.add_argument('--dir_protocolo', action = 'store', dest = 'dir_protocolo', default = 'outputs/saveSepto11/', required = False, help = 'Diretório onde os .ini finais encontrados vão ser armazenados, junto com os resultados do protocolo')
  ...
  while not end:
      ...
      if save:
          ...
          for d in range(len(deltas)):
              ...
              with open(caminho_arquivo, mode='a', newline='') as csv_file:
               csv_writer = csv.writer(csv_file, delimiter=',')
               csv_writer.writerow([sigma_factor, sigma, len(stim) - num_stim1 + 1, stim[-1] - stim[-2], taxa_espiral, taxa_bloqueio, taxa_normal, ('Bloqueio' if taxa_bloqueio > 50.0 else 'Propagação_Normal') if taxa_espiral<10 else 'Espiral'])
