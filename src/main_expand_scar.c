...
static void expand_scar_linear(const char *input, const char *output, int n_rows, char* n_volumes) {
    ...
    for (int i = 0; i < n_rows; i++) {
        ...
        FOR_EACH_CELL(grid) {
            ...
            if(i == 0) {
                ...
                evaluate = cell->active && extra_info[4] != 0 && extra_info[4] != 1 && !cell->visited;
            }
...
int main(int argc, char **argv) {
    ...
    expand_scar_linear(input, output, options->n_rows, options->n_volumes);
