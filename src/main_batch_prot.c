...
int main(int argc, char **argv) {
    ...
    for (int s = simulation_number_start; s < simulation_number_start + num_simulations; s++) {
        ...
        sprintf(command, "python  %s --sigma %f  --sigma_factor %f --output_dir_save /save --output_dir_restore /restore --nome_arquivo resultado.csv --dir_protocolo %s",batch_options->initial_config,
